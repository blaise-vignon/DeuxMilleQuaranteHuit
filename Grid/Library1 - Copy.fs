...
//                    if (Array2D.get g i j) = 0 then yield (i,j)]
        ...
        let zerolist = [ for j in [0] do
                          for i in  [0..3] do 
                                    if Array2D.get g (3-i) j = 0 then yield (3-i,j)
                          for j in [1] do
                                for i in  [0..3] do 
                                    if Array2D.get g i j = 0 then yield (i,j)
                          for j in [2] do
    ...
    let rndnew = fun _ ->  4//if rnd.Next(0,9) = 0 then 4 else 2
...
//            scoreRes
    ...
    let play g=
        let mutable long = false
        while not (isLost g) do
            long <- long || (g |> makeList |> List.sum > 1024+2048+4096+8192+16384)
...
//                if true then
                    ...
                    | n when n > 11 -> scoreList <- (dir,score 1 grid) :: scoreList
                    | n when n > 8 -> scoreList <- (dir,score 2 grid) :: scoreList
                    | n when n > 5 -> scoreList <- (dir,score 3 grid) :: scoreList
                    | n when n > 3 -> scoreList <- (dir,score 4 grid) :: scoreList
                    | _ -> scoreList <- (dir,score 7 grid) :: scoreList
            ...
            let elimLeft (d,s) = (d,s) //if (d=Left) then (d,s-1000.0) else (d,s)
